...
3. Copy the token — you'll store it as a CI secret

### Find your Endpoint ID

Your cluster's endpoint ID is visible in the Portainer URL when viewing the cluster, e.g. `https://portainer.example.com/#!/2/kubernetes/dashboard` — the endpoint ID is `2`.

### Pipeline Changes

The only difference from the standard pipeline is how kubeconfig is configured. Replace the "Configure kubeconfig" step with a step that fetches it from Portainer:

<Tabs>
<TabItem value="github" label="GitHub Actions" default>

Add `PORTAINER_ENDPOINT_ID` to the `env` section of your workflow:

```yaml
env:
  PORTAINER_ENDPOINT_ID: "2"           # your cluster's endpoint ID
```

Replace the "Configure kubeconfig" step with:
...
| `PORTAINER_TOKEN` | The access token you created above |

You no longer need the `KUBECONFIG` secret.

</TabItem>
<TabItem value="gitlab" label="GitLab CI/CD">

Add `PORTAINER_ENDPOINT_ID` to the `variables` section:
...
Replace the `before_script` in the deploy stage with:
...
Add these variables (**Settings > CI/CD > Variables**):

| Variable | Type | How to get it |
|---|---|---|
| `PORTAINER_URL` | Variable (masked) | Your Portainer instance URL (e.g. `https://portainer.example.com`) |
| `PORTAINER_TOKEN` | Variable (masked) | The access token you created above |

You no longer need the `KUBECONFIG` variable.

</TabItem>
</Tabs>

:::tip How it works
Portainer's `/api/kubernetes/config` endpoint returns a kubeconfig where the server URL points to Portainer's kubectl proxy (e.g. `https://portainer.example.com/api/endpoints/2/kubernetes`). All kubectl commands route through Portainer, which enforces access control and auditing. The rest of the pipeline (apply manifests, update image, rollout) works exactly the same.
:::
